...
namespace panda_controllers{
    ...
    bool Slotine::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& node_handle)
    {
        ...
        /* Chek corretta acquisizione dei parametri del Slotined*/
		...
		!node_handle.getParam("kv3", kv3)) {
		    ROS_ERROR("Slotine: Could not get parameter kpi or kv!");
		    return false;
	    }
    ...
    void Slotine::update(const ros::Time&, const ros::Duration& period){
     ...
     /*   // Publish tracking errors as joint states(ad ogni update nodo mi mostra il valore di errore di posizione e velocità ai giunti e l'istante di riferimento)
        ...
        tau_cmd = Mest * command_dot_dot_q_d + Cest * command_dot_q_d  + Kp_apix * error + Kv_apix * dot_error + Gest;// + Dest*command_dot_q_d; // perchè si sottrae G a legge controllo standard?  legge controllo Slotine (usare M,C e G dovrebbe essere la stessa cosa)
        // tau_cmd = Mest * command_dot_dot_q_d + Cest * dot_q_curr  + Kp_apix * error + Kv_apix * dot_error + Gest - G; // perchè si sottrae G a legge controllo standard?  legge controllo Slotine (usare M,C e G dovrebbe essere la stessa cosa)
